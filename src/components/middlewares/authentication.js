    ...
    async (payload, done) => {
      try {
        ...
        const user = userPayload(
          await userService.findById(parseUserId(payload.user_id)),
        );

        return done(
          null,
          { ...user, exp: payload.exp, jti: payload.jti } || false,
        );
      } catch (err) {
    ...
    async (payload, done) => {
      try {
        ...
        const admin = adminPayload(
          await adminService.findById(parseAdminId(payload.admin_id)),
        );

        return done(
          null,
          { ...admin, exp: payload.exp, jti: payload.jti } || false,
        );
      } catch (err) {
        return done(err, false);
      }
...
function isAccountVerified(req, res, next) {
  try {
    const message =
      "Anda tidak diizinkan mengakses fitur ini karena email Anda belum terverifikasi!";
    ...
    if (!account.verified)
      throw errorResponder(errors.ACCOUNT_UNVERIFIED, message);
